...
export class LobbyObserver {
  constructor(node) {
    ...
    SessionManager.getInstance().fetchSessions().then(() => {
      this.refreshLobbies();
    });
  ...
  handleMutations(mutationList) {
    ...
    mutationList.forEach(mutation => {
      if (mutation.type === 'childList') {
        ...
        Array.from(mutation.removedNodes).filter(node => node.nodeType === Node.ELEMENT_NODE).forEach(async (node) => {
          if (node.matches('div.loader')) {
            await SessionManager.getInstance().fetchSessions();
            shouldRefresh = true;
          }
        });
